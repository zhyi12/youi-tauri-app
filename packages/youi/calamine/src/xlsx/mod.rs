...
use crate::{Cell, CellErrorType, DataType, Dimensions, Formula, Metadata, Range, Reader, Sheet, SheetType, SheetVisible, Table};
...
impl<RS: Read + Seek> Reader<RS> for Xlsx<RS> {
    ...
    fn worksheet_formula(&mut self, name: &str) -> Result<Range<Formula>, XlsxError> {
        let mut cell_reader = self.worksheet_cells_reader(name)?;
        let len = cell_reader.dimensions().len();
        let mut cells = Vec::new();
        if len < 100_000 {
        ...
        while let Some(cell) = cell_reader.next_formula()? {
            //i//f !cell.val.expression.is_empty() || cell.val.ref_area.is_some() {
                cells.push(cell);
            //}
        }
        Ok(Range::from_sparse(cells))
