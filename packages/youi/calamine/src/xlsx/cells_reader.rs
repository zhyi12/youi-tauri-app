...
use crate::{datatype::DataTypeRef, formats::{format_excel_f64_ref, CellFormat}, Cell, XlsxError, Formula};
...
impl<'a> XlsxCellReader<'a> {
    ...
    pub fn next_formula(&mut self) -> Result<Option<Cell<Formula>>, XlsxError> {
        loop {
            self.buf.clear();
            match self.xml.read_event_into(&mut self.buf) {
                Ok(Event::Start(ref row_element))
...
fn read_formula<'s>(
    xml: &mut XlReader<'_>,
    e: &BytesStart<'_>,
) -> Result<Option<Formula>, XlsxError> {
    match e.local_name().as_ref() {
        ...
        b"f" => {
            ...
            //获取ref属性,公式扩展处理
            let ref_attribute = get_attribute(e.attributes(), QName(b"ref"))?;
            match ref_attribute {
                None => {}
                Some(attr) => {
                    attr_ref.push_str(std::str::from_utf8(attr).unwrap_or(""));
                }
            }
            ...
            Ok(Some(Formula::new(f).formula(attr_ref)))
        }
