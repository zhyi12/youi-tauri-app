...
use serde::{self, Deserialize,Serializer,Serialize};
...
impl Serialize for DataType {
    ...
    {
        match self {
            DataType::Int(i) => {serializer.serialize_i64(i.clone())}
            DataType::Float(x) => {serializer.serialize_f64(x.clone())}
            DataType::String(x) => {serializer.serialize_str(x)}
            DataType::Bool(x) => {serializer.serialize_bool(x.clone())}
            // DataType::DateTime(x) => {}
            // DataType::Duration(_) => {}
