...
use crate::{Cell, CellErrorType, DataType, Formula, Metadata, Range, Reader, Sheet, SheetType, SheetVisible};
...
pub struct Xls<RS> {
    sheets: BTreeMap<String, (Range<DataType>, Range<Formula>, Vec<Dimensions>)>,
...
impl<RS: Read + Seek> Reader<RS> for Xls<RS> {
    ...
    fn worksheet_formula(&mut self, name: &str) -> Result<Range<Formula>, XlsError> {
        self.sheets
            .get(name)
            .ok_or_else(|| XlsError::WorksheetNotFound(name.into()))
            .map(|r| r.1.clone())
            ...
            for record in records {
                ...
                match r.typ {
                    ...
                    0x0006 => {
                        ...
                        formulas.push(Cell::new(fmla_pos, Formula::new(fmla)));
                    }
                    _ => (),
                }
            }
