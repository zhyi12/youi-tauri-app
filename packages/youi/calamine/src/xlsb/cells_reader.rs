use crate::{datatype::DataTypeRef, formats::{format_excel_f64_ref, CellFormat}, utils::{read_f64, read_i32, read_u32, read_usize}, Cell, CellErrorType, Dimensions, XlsbError, Formula};
...
impl<'a> XlsbCellsReader<'a> {
    ...
    pub fn next_formula(&mut self) -> Result<Option<Cell<Formula>>, XlsbError> {
        let value = loop {
            self.typ = self.iter.read_type()?;
            let _ = self.iter.fill_buffer(&mut self.buf)?;
        ...
        let col = read_u32(&self.buf);
        Ok(Some(Cell::new((self.row, col), Formula::new(value))))
    }
}
