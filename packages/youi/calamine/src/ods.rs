...
use crate::{DataType, Formula, Metadata, Range, Reader, Sheet, SheetType, SheetVisible};
...
/// http://docs.oasis-open.org/office/v1.2/OpenDocument-v1.2.pdf
pub struct Ods<RS> {
    sheets: BTreeMap<String, (Range<DataType>, Range<Formula>)>,
    metadata: Metadata,
    ...
    RS: Read + Seek,
{
    ...
    /// Read worksheet data in corresponding worksheet path
    fn worksheet_formula(&mut self, name: &str) -> Result<Range<Formula>, OdsError> {
        self.sheets
            .get(name)
            .ok_or_else(|| OdsError::WorksheetNotFound(name.into()))
            .map(|r| r.1.to_owned())
...
fn read_table(reader: &mut OdsReader<'_>) -> Result<(Range<DataType>, Range<Formula>), OdsError> {
        ...
        match reader.read_event_into(row_buf) {
                    ...
                    || e.name() == QName(b"table:covered-table-cell") =>
            {
                ...
                for _ in 0..empty_col_repeats {
                    ...
                    formulas.push(Formula::new("".to_string()));
                }
                empty_col_repeats = 0;
                ...
                if value.is_empty() && formula.is_empty() {
                    ...
                    for _ in 0..repeats {
                        ...
                        formulas.push(Formula::new(formula.clone()));
                    }
                }
                if !is_closed {
