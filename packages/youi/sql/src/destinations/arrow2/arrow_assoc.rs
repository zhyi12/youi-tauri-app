...
impl ArrowAssoc for DateTime<Utc> {
    ...
    fn push(builder: &mut Self::Builder, value: DateTime<Utc>) {
        builder.push(Some(value).map(|x| x.timestamp_nanos_opt().unwrap()));
    }
...
impl ArrowAssoc for Option<DateTime<Utc>> {
    ...
    fn push(builder: &mut Self::Builder, value: Option<DateTime<Utc>>) {
        builder.push(value.map(|x| x.timestamp_nanos_opt().unwrap()));
    }
...
//         builder.push(Some(value).map(|x| x.and_hms_opt(0, 0, 0).unwrap().timestamp()))
//     }
//
//     fn field(header: &str) -> Field {
//         Field::new(
...
//         builder.push(value.map(|x| x.and_hms_opt(0, 0, 0).unwrap().timestamp()))
//     }
//
//     fn field(header: &str) -> Field {
//         Field::new(
...
fn naive_date_to_date32(nd: NaiveDate) -> i32 {
    (nd.and_hms_opt(0, 0, 0).unwrap().and_utc().timestamp() / SECONDS_IN_DAY) as i32
}
...
impl ArrowAssoc for Option<NaiveDateTime> {
    ...
    fn push(builder: &mut Self::Builder, value: Option<NaiveDateTime>) {
        builder.push(value.map(|x| x.and_utc().timestamp_nanos_opt().unwrap()));
    }
...
impl ArrowAssoc for NaiveDateTime {
    ...
    fn push(builder: &mut Self::Builder, value: NaiveDateTime) {
        builder.push(Some(value).map(|x| x.and_utc().timestamp()));
    }
